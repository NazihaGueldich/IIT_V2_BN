...
<div class="container-xxl flex-grow-1 container-p-y">
    <br />
    <h1>Détails du document demandé</h1>
    <br />
    <div class="row">
                    ...
                    @switch($document[0]->etat)
                        @case(0)
                            <form action="{{ route('documents.update', ['document' => $document[0]->id, 'etat' => '1']) }}"
                                method="post" style="display: inline-block; margin-right: 10px;padding-left: 40px;">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="btn btn-success">Prêt</button>
                            </form>
                            <form action="{{ route('documents.update', ['document' => $document[0]->id, 'etat' => '2']) }}"
                                method="post" style="display: inline-block; margin-right: 10px; padding-left: 50px;">
                                @csrf
                                @method('PUT')
        ...
        <div class="col-md-6 col-xl-6">
            <div class="card shadow-none bg-transparent border border-secondary mb-3">
                <div class="card-body">
                    ...
                    <p class="card-text">
                        <strong>Nom :</strong>
                        {{ $document[0]->student->nom }}
                    </p>
                    <p class="card-text">
                        <strong>Prénom :</strong>
                        {{ $document[0]->student->prenom }}
                    </p>
                    <p class="card-text">
                        <strong>Nationalité :</strong>
                        {{ $document[0]->student->nationalite }}
                    </p>
                    <p class="card-text">
                        <strong>Email :</strong>
                        {{ $document[0]->student->email }}
                    </p>
                    <p class="card-text">
                        <strong>Télephone :</strong>
                        {{ $document[0]->student->tel_mobile }}
                    </p>
                </div>
            </div>
        </div>
